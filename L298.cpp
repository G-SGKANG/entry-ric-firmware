...
Mechatro::Mechatro(int pwmEnable, int dirPIN)
{
  this->attach(pwmEnable, dirPIN);
}

void Mechatro::attach(int pwmEnable, int dirPIN)
{
  _pwmEnable = pwmEnable;
  _dirPIN = dirPIN;
  _brakePIN = -1; // The Brake mode is not used
  pinMode(_pwmEnable, OUTPUT);
  pinMode(_dirPIN, OUTPUT);
  digitalWrite(_dirPIN, 1); // 0:Reverse, 1:Forward
}
...
/*
  ■ pwmVAL = motor speed 0~100
  ■ dir 0:Reverse, 1:Forward
*/
void Mechatro::speed(int pwmVal, byte dir)
{
  if (_brakePIN > 0)
  {
    digitalWrite(_brakePIN, 0);
